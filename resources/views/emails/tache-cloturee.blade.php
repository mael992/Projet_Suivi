...
.header { background:#1f2937; border-bottom:3px solid #16a34a; padding:20px 24px; color:#fff; font-size:18px; font-weight:bold; }
.body { padding:28px 24px; }
.info { background:#f0fdf4; border-left:4px solid #16a34a; padding:14px 18px; border-radius:4px; margin:16px 0; font-size:14px; }
...
<div class="card">
    ...
    <div class="body">
        ...
        <p>✅ La tâche <strong>{{ $tache->reference }}</strong> vient de passer au statut
        <strong>« Fait »</strong>.</p>

        <div class="info">
            <div>🔖 <strong>Référence :</strong> {{ $tache->reference }}</div>
            <div>👥 <strong>Service :</strong> {{ $tache->service_label }}</div>
            @if($tache->assigne)<div>👤 <strong>Réalisée par :</strong> {{ $tache->assigne->username }}</div>@endif
            <div>🕐 <strong>Clôturée le :</strong> {{ $tache->date_cloture?->format('d/m/Y à H:i') }}</div>
            @if($tache->description_cloture)<div>📝 {{ Str::limit($tache->description_cloture, 200) }}</div>@endif
        </div>
